...
def load_metadata_dict(fname):
    ...
    with h5py.File(fname, 'r') as hf:
        ...
        if len(hf['policy']['actions'].shape) == 1: # when single time-steps saved in hdf5 instead of video
            meta_data_dict['sdim'] = hf['env']['state'].shape[0]
            meta_data_dict['adim'] = hf['policy']['actions'].shape[0]
            meta_data_dict['frame_dim'] = hf['env']['image'].attrs['shape'][:2]
        else:  # whole video saved in hdf5:
